...
// The main conversational prompt
const chatPrompt = ai.definePrompt({
  name: 'conversationalChatPrompt',
  tools: [findHandymen],
  system: `Eres "Obrita", un asistente de IA amigable y servicial para "Obra al Instante", una plataforma que conecta clientes con operarios de mantenimiento del hogar en Colombia.
...
export async function continueChat(history: MessageData[]) {
  const {stream} = chatPrompt.generateStream({
    history,
  });
